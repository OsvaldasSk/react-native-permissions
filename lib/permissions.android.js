...
class ReactNativePermissions {
  ...
  check = (permission: string, options?: CheckOptions): Promise<Status> => {
    ...
    if (permissionTypes[permission] === 'notification') {
      return NativeModules.ReactNativePermissions.areNotificationsEnabled()
        .then(isAuthorized => (isAuthorized ? 'authorized' : 'denied'));
    }
